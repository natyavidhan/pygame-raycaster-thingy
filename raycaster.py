    ...
    def run(self):
        while self.running:
            ...
            if keys[pygame.K_d]:
                self.player.angle += (0.05*delta_time)
            if keys[pygame.K_w]:
                nextX = self.player.x + -math.sin(self.player.angle) * self.player.speed * delta_time
                nextY = self.player.y + math.cos(self.player.angle) * self.player.speed * delta_time
